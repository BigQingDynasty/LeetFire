...
func Constructor(root *TreeNode) BSTIterator {
	iter := BSTIterator{}
	iter.head = &TreeNode{}
	iter.head.Right = iter.traversal(root)
	return iter
}

func (this *BSTIterator) traversal(root *TreeNode) (head *TreeNode) {
	if root == nil {
		return nil
	}
	...
	if root.Left != nil {
		lh := this.traversal(root.Left)
		head = lh
		for ; lh.Right != nil; lh = lh.Right {
		}
		lh.Right = root
	} else {
		head = root
